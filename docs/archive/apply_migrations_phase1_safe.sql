...
-- 2. Drop old CHECK constraint (try common auto-generated names)
ALTER TABLE leads DROP CONSTRAINT IF EXISTS leads_stage_check;
ALTER TABLE leads DROP CONSTRAINT IF EXISTS leads_stage_stage_check;
ALTER TABLE leads DROP CONSTRAINT IF EXISTS leads_stage_check1;
...
CREATE POLICY "crm_workers_read_stage_history" ON public.lead_stage_history
...
CREATE POLICY "crm_workers_insert_stage_history" ON public.lead_stage_history
...
CREATE POLICY "crm_workers_insert_tasks" ON public.tasks
  FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid()
...
CREATE POLICY "crm_workers_update_tasks" ON public.tasks
...
CREATE POLICY "crm_workers_delete_tasks" ON public.tasks
