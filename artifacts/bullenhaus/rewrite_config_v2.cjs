...
// Insert the admin keys at the end of the EN `common` block
// The `en` block ends right before `de: { common: { dashboard: 'Dashboard'`
const enCommonEnd = code.indexOf("de: {");
const enBlock = code.substring(0, enCommonEnd);
const newEnBlock = enBlock.replace(/(\n\s*\}\n\s*\}\n\s*),?\n\s*$/, ",\n" + adminEN + "$1,\n        crm: crmDictionaries.en\n      },\n      ");

const deBlock = code.substring(enCommonEnd);
// The `de` block ends right before `i18n.on('languageChanged'`
const deCommonEnd = deBlock.indexOf("i18n.on('languageChanged'");
const deContent = deBlock.substring(0, deCommonEnd);
const newDeBlock = deContent.replace(/(\n\s*\}\n\s*\}\n\s*),?\n\s*\}\n\s*\}\);\n\s*$/, ",\n" + adminDE + "$1,\n        crm: crmDictionaries.de\n      }\n    }\n  });\n\n");

code = newEnBlock + newDeBlock + "i18n.on('languageChanged'" + deBlock.substring(deCommonEnd + "i18n.on('languageChanged'".length);
